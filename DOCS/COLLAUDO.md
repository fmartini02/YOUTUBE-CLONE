...
- [x] **Ricerca di playlist** — `GET /api/search?q=...&tipo=playlist` restituisce solo playlist (id
      ...
      "Soul Lo Fi" con id. Dopo il merge `tipo=playlist` è servito dall'implementazione dei filtri (#14),
      stessa `sp`; il filtro nella UI è quello del pannello "Filtri → Tipo".
...
- [x] **Preferenze** — `GET /api/prefs` torna almeno `quality`, `autoplay`, `theme`, `fitScreen`, `resume`, `sponsorBlock`, `sponsorCategories`;
