 ...
 * senza logo per tutta la sessione — collegarsi non li faceva comparire e
 * ricaricare la pagina era l'unico rimedio. Ora un id torna richiedibile se la
...
export function useChannelAvatars(videos, authKey) {
  ...
  useEffect(() => {
    ...
    api.channelAvatars(ids).then(d => {
      ...
      // Chi non è stato risolto può essere ritentato più avanti (il server
      // tiene già una cache negativa, quindi non è una richiesta sprecata:
      // risponde senza richiamare YouTube).
