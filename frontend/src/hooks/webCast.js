...
/**
 * Chromecast via Cast Web Sender SDK di Google — il ramo desktop di useCast.
 *
 ...
 * bottone (vedi CastButton). Il telecomando completo (oggetto `remote`) esiste
 * solo sul ramo nativo dell'APK (hooks/nativeCast.js): qui `remote` è null.
 *
...
export function useWebCast() {
  ...
  return { ...state, remote: null, startCast, stopCast };
}
