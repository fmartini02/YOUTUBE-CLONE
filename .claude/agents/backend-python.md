---
name: backend-python
description: Implementa modifiche al server Python di YTProxy — endpoint FastAPI in server/routers/ (un modulo per dominio, composti da server/main.py), estrazione yt-dlp e stato in server/auth/ (un modulo per dominio, stato condiviso in auth/state.py), scheduler in server/sync/, patch estrattore in server/ytdlp/patch.py. Usalo per aggiungere o correggere endpoint, feed, riproduzione/mux, autenticazione cookie o OAuth.
...
- **I nuovi endpoint di scrittura sono già protetti** dal middleware `blocca_scritture_esterne` (`core/security.py`): non aggiungere controlli d'origine a mano, e non aggirarlo.
...
- Ogni patch in `ytdlp/patch.py` agisce **solo se il dato manca**, così non fa danni quando yt-dlp si aggiorna.
