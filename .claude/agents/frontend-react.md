...
- **Aggiungere una pagina sono tre modifiche coerenti**: `pageToUrl`/`urlToPage` in `App.jsx`, il rendering condizionale in `App.jsx`, e la route in `spa_routes()` di `server/routers/spa.py`. Senza la terza, aprire l'URL diretto o ricaricare dà 404.
...
- **Stile 42: non più di 5 funzioni/componenti per file, 25 righe di codice per funzione, 5 argomenti/prop-destrutturate per funzione** (vedi CLAUDE.md; eccezione per i file di puro stile). Nessuno script verifica il frontend — controlla a mano prima di dire che una modifica è conforme. Se il vincolo confligge con un ordine di effect che ha una ragione documentata (es. `playbackRate` dopo `load()` in `VideoPlayer.jsx`), vince la correttezza: lascia il file oltre soglia con un commento che spiega perché, non riordinare gli hook per far quadrare il conteggio.
