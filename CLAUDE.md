...
In sola copia un flusso non può partire da metà GOP, e le due scorciatoie ovvie sbagliano entrambe: `-ss` da solo porta al keyframe *precedente* (video subito, ma se lo stesso `-ss` va anche all'audio i due partono da secondi diversi e restano fuori sincrono), `-copypriorss 0` aspetta il keyframe *successivo* — diversi secondi di caricamento su ogni salto in avanti, e vicino alla fine del video, dove un keyframe dopo non esiste, un flusso **senza traccia video** che pianta anche l'audio (era il bug "verso la fine perdo l'audio" + "quando mando avanti rimane a caricare"). Oggi `_keyframe_before()` (`server/routers/streaming.py`) fa un `ffprobe` mirato (finestra 15s, pochi KB di range request) per trovare l'ultimo keyframe ≤ `start` e passa **quello** come `-ss` a video e audio insieme: video immediato, audio in sincrono, mai un buco. Costo: il player mostra `start + currentTime` mentre il contenuto vero parte dal keyframe, quindi la barra è avanti di ≤ 1 GOP rispetto a quello che si vede (non deriva nel tempo). Se ffprobe fallisce si ricade sul `start` chiesto così com'è. `/api/watch` (`server/routers/watch.py`) serve solo ai metadati della pagina video: nessun URL di stream, li risolve `/api/mux` da sé tenendoli in cache per il seek.
