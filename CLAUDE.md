...
In sola copia un flusso non può partire da metà GOP, e le scorciatoie ovvie sbagliano: `-copypriorss 0` aspetta il keyframe *successivo* — diversi secondi di caricamento su ogni salto in avanti, e vicino alla fine del video, dove un keyframe dopo non esiste, un flusso **senza traccia video** che pianta anche l'audio (era il bug "verso la fine perdo l'audio" + "quando mando avanti rimane a caricare"). `-ss` prima di un input in `-c:v copy` atterra da sé sull'ultimo keyframe ≤ quel valore (primo pacchetto video sempre `K`, verificato). Il player mostra `start + currentTime`, quindi dopo un salto la barra può essere avanti di ≤ 1 GOP rispetto al fotogramma — offset fisso, non deriva, come lo snap ai segmenti di YouTube; su YouTube i GOP misurati vanno da <1s a ~2.7s, non un valore fisso.

Passare lo stesso `-ss` grezzo a video e audio **non basta**: con `-c copy` su due input HTTP separati, ognuno col proprio `-ss` e range request, l'MP4 esce con un buco *deterministico* di ~9s senza pacchetti video subito dopo il keyframe (l'audio scorre) e il `<video>` resta a bufferare all'infinito — succede anche con `-ss` esatto sul keyframe, è l'interleaver di ffmpeg che in sola copia non sincronizza i due flussi appena aperti. La cura, **solo sui salti** (`start>0`, ramo MP4), è ricodificare la sola traccia audio: `-c:v copy -c:a aac -b:a 160k` in `_build_ffmpeg_cmd` (AAC = encoder nativo, sempre presente; ~130 kbit/s, costo trascurabile anche su Raspberry). Verificato: 6/6 salti puliti contro 6/6 col buco. Ma questo da solo introduce un secondo problema, più subdolo: l'audio ricodificato (decodificato e accurate-seek) atterra quasi esatto sul valore grezzo di `-ss`, mentre il video in copia resta sul keyframe *precedente* — fino a un GOP prima. Il muxer fMP4 rimappa ciascuno stream a pts 0 indipendentemente (confermato: `-copyts` non lo impedisce su questo muxer, l'`avoid_negative_ts` per l'output frammentato lo ignora), quindi i due flussi restano permanentemente sfasati per TUTTO il resto della riproduzione — non il semplice offset "barra avanti sul fotogramma" del paragrafo sopra, un vero e udibile disallineamento audio/video (misurato 0.6-2.7s su YouTube). `_keyframe_before()` (un `ffprobe` mirato per trovare quel keyframe e darlo a ENTRAMBI gli input) era stato rimosso perché costava 4-5s sul percorso critico (finestra intera, secondi di dati) — **reintrodotto** in forma leggera: `read_intervals "target%+#3"` legge solo 3 fotogrammi invece di una finestra, ~0.7-1s misurati (contro 4-5s prima). Il salto paga quel costo solo su `start>0`; `start=0` non lo chiama.

`start=0` e `/api/download` restano `-c copy` senza probe (nessun mismatch da correggere, non c'è audio ricodificato). Il ramo `webm` (Cast 4K) resta in copia (l'AAC non entra in un WebM — da riprovare con un ricevitore reale) e altrettanto senza probe. `/api/watch` (`server/routers/watch.py`) serve solo ai metadati della pagina video: nessun URL di stream, li risolve `/api/mux` da sé tenendoli in cache per il seek.

**Limite noto, non risolto**: la barra di caricamento (`.player-progress-buffer`) può restare ferma anche con il video in pausa a lungo — verificato con Chromium reale: `video.buffered` non cresce oltre quanto già scaricato quando la riproduzione è ferma, perché il flusso non ha `Content-Length` né durata dichiarata nel contenitore e Chrome tratta la risorsa in modo simile a un flusso live (non fa readahead oltre un minimo mentre è in pausa). `preload="auto"` dichiarato sul `<video>` non basta (verificato: nessun effetto). Una cura vera richiederebbe passare a MediaSource Extensions (bufferizzazione gestita da JS invece che dal browser) — un cambio di architettura, non una riga di codice.
